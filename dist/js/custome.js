...
$(document).ready(function () {
  $.ajax({
    type: "GET",
    url: "http:/10.10.30.45/jzu/public/api/v1/system/cost_center_title/get",
    dataType: "json",
...
$(document).ready(function () {
  $.ajax({
    type: "GET",
    url: "http:/10.10.30.45/jzu/public/api/v1/system/cost_center_code/get",
    dataType: "json",
    success: function (data) {
      var items = JSON.stringify(data.data);
      items = items.replace("[");
...
$(document).ready(function () {
  $.ajax({
    type: "GET",
    url: "http:/10.10.30.45/jzu/public/api/v1/system/official_work_section/get",
    dataType: "json",
    success: function (data) {
      var items = JSON.stringify(data.data);
      items = items.replace("[", "");
...
function onchangeKalan() {
  ...
  $(document).ready(function () {
    $.ajax({
      type: "GET",
      url: "http:/10.10.30.45/jzu/public/api/v1/system/levels/get?levelID=" + k,
      dataType: "json",
      success: function (data) {
...
function onchangeGroh() {
  ...
  $(document).ready(function () {
    $.ajax({
      type: "GET",
      url: "http:/10.10.30.45/jzu/public/api/v1/system/levels/get?levelID=" + g,
      dataType: "json",
      success: function (data) {
...
function onchangeKarbarg() {
  ...
  $(document).ready(function () {
    $.ajax({
      type: "GET",
      url:
        "http:/10.10.30.45/jzu/public/api/v1/system/levels/get?levelID=" + kb,
      dataType: "json",
      success: function (data) {
...
$(document).ready(function () {
  $.ajax({
    type: "GET",
    url: "http:/10.10.30.45/jzu/public/api/v1/system/levels/get",
    dataType: "json",
    success: function (data) {
...
function getViewTest() {
  $(document).ready(function () {
    $.ajax({
      ...
      dataType: "json",
      success: function (data) {
        const testList = JSON.parse(this.responseText);
        for (var i = 0; i < testList.length; i++) {
          $(".rowNew").prepend("<div>" +  testList[i] + "</div>");
        }
      },
    });
  });
...
function getView1() {
  $(document).ready(function () {
    ...
    request.open("GET", "http://10.10.30.45/jzu/public/api/v1/activities/1");
...
function getView2() {
  $(document).ready(function () {
    var request = new XMLHttpRequest();
    request.open("GET", "http://10.10.30.45/jzu/public/api/v1/activities/2");
    request.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        generateCateBox(JSON.parse(this.responseText));
      }
...
function getView3() {
  $(document).ready(function () {
    var request = new XMLHttpRequest();
    request.open("GET", "http://10.10.30.45/jzu/public/api/v1/activities/3");
    request.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        generateCateBox(JSON.parse(this.responseText));
      }
...
function sabtFraiand() {
  $(document).ready(function () {
    ...
    $.ajax({
      type: "POST",
      url: "http:/10.10.30.45/jzu/public/api/v1/activity",
      dataType: "json",
      accept: "application/json",
...
function SaveAndSend() {
  $(document).ready(function () {
    $.ajax({
      type: "POST",
      url: "http:/10.10.30.45/jzu/public/api/v1/activity/sub_activity",
      dataType: "json",
      accept: "application/json",
      ContentType: "application/json",
      XCSRFTOKEN: "",
      ...
      success: function (data) {
        /* 
        $(".actvity").val(data.data.activity)
        $(".subac").val(data.data.subActivity)
        var b1 = $("#resCodeSazman").val();
        var b2 = $(".resCodeSazman").val(b1);
      
        var c1 = $("#selectNumber").val();
        var c2 = $(".selectNumber").val(c1);
      
        var d1 = $("#rezTitlesazman").val();
        var d2 = $(".rezTitlesazman").val(d1);
      
        var e1 = $("#codeFaaliat").val();
        var e2 = $(".codeFaaliat").val(e1);
      
        var f1 = $("#shenasehFaraiand").val();
        var f2 = $(".shenasehFaraiand").val(f1);
      
        var j1 = $("#titleFaraiand").val();
        var j2 = $(".titleFaraiand").val(j1);
      
        var g1 = $("#paiesh").val();
        var g2 = $(".paiesh").val(g1);
      
        var u1 = $("#ahdaf").val();
        var u2 = $(".ahdaf").val(u1);
        
        var n1 = $("#titleVorod").val();
        var n2 = $(".titleVorod").val(n1);
      
        var m1 = $("#mabdaeVorod").val();
        var m2 = $(".mabdaeVorod").val(m1);
      
        var z1 = $("#titleMojre").val();
        var z2 = $(".titleMojre").val(z1);
      
        var x1 = $("#titleKhoroje").val();
        var x2 = $(".titleKhoroje").val(x1);
      
        var r1 = $("#maghsadKhoroje").val();
        var r2 = $(".maghsadKhoroje").val(r1);
      
        var t1 = $("#noewRavesh").val();
        var t2 = $(".noewRavesh").val(t1);
      
        var y1 = $("#nameSamaneh").val();
        var y2 = $(".nameSamaneh").val(y1);
      
        var i1 = $("#departmentsDropdown").val();
        var i2 = $(".departmentsDropdown").val(i1);
      
        var o1 = $("#marahelFaaliat").val();
        var o2 = $(".marahelFaaliat").val(o1);
      
        var p1 = $("#sharhFaaliat").val();
        var p2 = $(".sharhFaaliat").val(p1);
        
        var h1 = $("#shenaseMojre").val();
        var h2 = $(".shenaseMojre").val(h1);
      
        var k1 = $("#mostanadatJare").val();
        var k2 = $(".mostanadatJare").val(k1);
      
        var v1 = $("#poshtiban").val();
        var v2 = $(".poshtiban").val(v1);
      */
      },
    });
  });
  var titF = $("#titleFaraiand").val();
  var pai = $("#paiesh").val();
  ...
  if (titF == "") {
    $("#titleFaraiand").css("border", "2px solid red");
    $(".titFar").text("لطفا عنوان فرآیند را پر نمائید").css("color", "red");
    Swal.fire({
    ...
    $(".paiesh").text("لطفا شناسه پایش را پر نمائید").css("color", "red");
    Swal.fire({
      text: "لطفا شناسه پایش را وارد نمائید",
      icon: "warning",
      confirmButtonText: "ok",
    });
  } else if (titV == "") {
    ...
    Swal.fire({
      ...
      icon: "success",
      confirmButtonText: "<a style='color:#fff'>ثبت کاربرگ جدید</a>",
    });
